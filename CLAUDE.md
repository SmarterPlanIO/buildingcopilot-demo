...
> Derniere mise a jour : 10 septembre 2026. Harness Streamlit v0.8.0 (mode agent), backend MCP **image v12** (fiche v2 annuaire) sur les 3 Lambdas depuis le 01/09, **rollouts fiche v2 termines sur les 3 tenants le 10/09** (sequence obligatoire : 06a par tenant AVANT 09b — les images sont communes, les schemas RDS non). Project Instructions : NCG v4.0, Delacour **v1.3** (fusion doctrine fiche + skills dlc-*), CSG v1.1 — instanciees depuis le template produit `Scripts/clients/INSTRUCTIONS_TEMPLATE_PALIM.md`. Tenants : NCG (pilote), Delacour Patrimoine (branche orpheline a parite v12, **livraison client le 11/09**), CSG (courtier, 1 copro).
>
...
- **Modele de livraison** : le client (NCG) utilise son propre LLM (Claude Teams / Cowork) avec des **Project Instructions** fournies (`Scripts/clients/ncg/docs/INSTRUCTIONS_NCG_PROJECT.md`, v3.4 du 26/08, version ecrite au seul Bloc 0, instanciees depuis le template produit `Scripts/clients/INSTRUCTIONS_TEMPLATE_PALIM.md`) et 5 **skills** (`ncg-redaction-livrable`, `ncg-note-juridique`, `ncg-fiche-decision`, `ncg-analyse-portefeuille`, `assynco-erp`). Ce LLM se connecte au **serveur MCP PALIM** qui expose le RAG documentaire et l'ERP assurance en tools.
- **Serveur MCP** : FastMCP (Python 3.12) sur **AWS Lambda** (image container + Lambda Web Adapter, `response_stream`), code dans `Scripts/mcp_server/`. **13 tools** `PALIM_*` (retrieval, analytique inter-copro, dossiers, synthese copro, visite 3D, Assynco, feedback). Authless : barriere = slug d'URL secret (`MCP_URL_SLUG`) plus resource policy de la Function URL. `stateless_http=True` obligatoire en Lambda. Compte AWS 046004768626, 3 fonctions (`palim-mcp` NCG, `palim-delacour-mcp`, `palim-csg-mcp`), **image v12 sur les trois** (v11 = soft delete `retrieval_exclu` + perimetres nommes ; v12 = fiche v2 annuaire — runbook `ops/runbooks/RUNBOOK_DEPLOY_V12.md` CLOS, rollouts v2 termines sur les 3 tenants le 10/09, sequence 06a par tenant documentee). Regle deploy : toujours un tag NEUF, jamais ecraser un tag existant (post-mortem v9).
...
- **Streamlit** : devenu **harness de debug interne** (toujours deploye depuis `main`, version dans `Scripts/Streamlit Cloud/VERSION` = 0.8.0). Mode **agent** depuis le 25/08 : boucle agentique Converse branchee sur le MCP backend unique (tools live + skills + prompt caching + pieces jointes), recette P3 = 9 scenarios PASS en prod. N'est plus le produit livre au client. La regle 3.1 (UI seulement) reste valide.
...
- **Rattrapage dedup — TERMINE (27-28/08)** : re-run `03->06b` fait sur Delacour (18 copros, +1 341 documents recuperes, verification 99,77 % de paires legitimes) et NCG (11 copros, factorisation publipostage seuil 0,70 exacte au chunk pres). Registres rafraichis (DOUBLON_PROCHE Delacour 1 921 -> 334). Residu accepte : ~180 doublons inter-dossiers (dossiers d'export, 0,23 %), la dedup proche de 03 ne compare qu'au sein d'un meme dossier.
- **Registre d'ingestion P1-P3** : ecritures a chaud dans 01/00b/02/03/06b via `registre.py` (a creer), rapport `ingest.py --rapport`, bascule detection suppressions. Plan : `Scripts/PLAN_REGISTRE_INGESTION.md` (P0 livre 20/08 + fix NFC Mac b203b03).
- **Analytique inter-copro — LIVRE** : `PALIM_run_analytical_query` (13e tool) + `PALIM_analytics.py`, deploye (image v10 puis v11), perimetres nommes + flotte demo 9 copros NGE (Bloc 13, b7a166a), Instructions v3.4. Verifier que v3.4 est bien recollee cote Claude Teams NCG (echo de version Bloc 0 en conversation neuve). Phase 2 (registre gestionnaire) non commencee.
- **Streamlit agentique (option A)** : P0-P2bis livres (v0.8.0 : boucle Converse + tools MCP live + skills + pieces jointes), recette P3 = 9 scenarios PASS en prod (7d11664). RESTE : P4 secret mcp sur Streamlit Cloud.
- **Scale ingestion 150 copros** : `raw_dir` per-copro (lecture directe share VPN/UNC, 6fb68e9) et `add_copro.py` + skill onboarding livres. RESTE : gate chunk-level dans `ingest.py` (propager les pures modifications), cleanup disque des shards, 08 per-copro, registre `copros` peuple par 08. Plan : `Scripts/PLAN_SCALE_150_COPROS.md`.
- **Multi-tenant** : Delacour Patrimoine PRET A LIVRER (11/09) — branche orpheline a parite v12 (3 ecarts volontaires seulement), Instructions v1.3, 5 skills dlc-*/assynco-erp, 25 copros / 137 994 chunks (105 av. de Verdun ingeree le 26/08), fiches v2 recettees ; CSG Cabinet Saint Germain (courtier, 1 copro Lacepede, fiche v2 rollee 10/09). Onboarding : skill `palim-onboarding-tenant`. Regle : les reinstanciations d'instructions Delacour partent de la v1.3 (la v1.2 avait perdu le cablage des skills, lecon du 10/09).
- **Plans rediges le 27/08, rien code** : citations cliquables (`Scripts/PLAN_CITATIONS_CLIQUABLES.md`, liens vers le partage Windows NCG) et self-learning (`Scripts/PLAN_SELF_LEARNING.md`, feedback Langfuse -> triage F1-F5 -> golden cases rejoues a chaque deploy ; P0 = contrat v2 `PALIM_log_feedback` a embarquer dans la future image v12).
