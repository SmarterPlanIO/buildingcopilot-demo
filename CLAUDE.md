...
> Derniere mise a jour : 27 aout 2026. Harness Streamlit v0.8.0 (mode agent), backend MCP image v11 (2 Lambdas : NCG le 24/08, Delacour le 26/08), Project Instructions NCG v3.4 du 26/08 (instanciees depuis le template produit `Scripts/clients/INSTRUCTIONS_TEMPLATE_PALIM.md`). Tenants : NCG (pilote), Delacour Patrimoine (worktree + branche dedies), CSG (courtier, 1 copro).
>
...
- **Rattrapage dedup — fix LIVRE, re-run a confirmer** : dedup proche 2 etages commitee+mergee (6863e3d + vetos dates 9ea9708, 21/08). Soft delete versions anterieures/variantes (brique 2+3) LIVRE et DEPLOYE image v11 (59df1b7 : flag `retrieval_exclu`, patron BORDEREAU_AR ; 1 597 chunks NCG + 57 Delacour flagues au deploy). RESTE A CONFIRMER : le re-run de rattrapage `03->06b` des copros touchees par DOUBLON_PROCHE (~18 600 chunks Delacour ; instructions agent redigees dans d247f3e, execution non tracee dans le repo).
...
- **Multi-tenant** : Delacour Patrimoine (worktree + branche orpheline dedies 9c91fce, Instructions v1.0, ingestion 105 av. de Verdun AE8711459 debloquee 26/08, ~3 813 fichiers a ingerer) ; CSG Cabinet Saint Germain (courtier, Instructions v1.0, 1 copro Lacepede). Onboarding : skill `palim-onboarding-tenant`.
