...
- **Analytique inter-copro en MCP — CODE (21/08/26)** : `PALIM_run_analytical_query` (13e tool) + `mcp_server/PALIM_analytics.py` (whitelist/builder = copie maitrisee d'analytics.py, SQL pur read-only, zero Bedrock, coverage + facettes). Tests contrats `tests/test_palim_analytics_contracts.py` (parite whitelist+builder avec Streamlit) + smoke live valides. Instructions v3.0 (Bloc 2 scinde + Bloc 12). RESTE : deploy image v10 (`mcp_server/RUNBOOK_DEPLOY_V10.md`) + recette live + recoller v3.0 cote Claude Teams.
