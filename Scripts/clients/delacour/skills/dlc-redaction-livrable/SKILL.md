---
name: dlc-redaction-livrable
description: >-
  Met en forme un livrable écrit pour un gestionnaire de copropriété Delacour Patrimoine : note
...
# Rédaction de livrable — Assistant Copro Delacour Patrimoine
...
- **Périmètre** : la/les copro(s) concernée(s) sont identifiées (immatriculation RNIC). Sinon, reviens au workflow de scope avant de rédiger.
...
## 6. Identité visuelle Delacour Patrimoine (logo)
Les livrables **externes** (courrier, note au conseil syndical) et **tout export Word** portent l'en-tête Delacour Patrimoine **avec le logo du client**. Le logo est le fichier **`logo Delacour.png`** présent dans les fichiers du Projet Claude. Place-le dans l'en-tête du document.
- Si `logo Delacour.png` n'est pas disponible dans le projet, **signale-le** (« logo Delacour.png introuvable dans les fichiers du projet ») et laisse un emplacement réservé en en-tête, plutôt que d'omettre le logo en silence.
...
- Si oui : génère un `.docx` propre — **logo Delacour dans l'en-tête**, titres hiérarchisés (Heading 1/2/3), listes à puces et numérotées, tableaux Word natifs, **sans aucun élément interne**. Propose-le en téléchargement.
