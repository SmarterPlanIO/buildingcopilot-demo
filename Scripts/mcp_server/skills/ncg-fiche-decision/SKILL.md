...
description: >-
  ...
  question juridique pure (→ ncg-note-juridique), ni pour la simple mise en forme
  d'un contenu déjà établi (→ ncg-redaction-livrable). La fiche propose ; elle ne
  décide jamais à la place des organes de la copropriété.
---

# Fiche de décision — Assistant Copro NCG
...
d'**orchestration** : la substance juridique vient de `ncg-note-juridique`, la mise
en forme et l'export de `ncg-redaction-livrable`, les données d'assurance live de
...
- **Périmètre copro fixé** (code NCG) avant toute instruction — invariant des
...
- **Tout est sourcé** : chaque montant, date, clause, résolution citée suit les
  règles de traçabilité de `ncg-redaction-livrable` (§4). Un devis n'est pas une
  décision ; un diagnostic n'est pas un vote.
- **Pas de conclusion juridique durcie** : majorités, délais et validité portent
  les réserves de `ncg-note-juridique` ([à vérifier contre le texte en vigueur]).
...
d. **Impact financier et répartition** : montants sourcés tels quels ; si la clé
   ...
   non-décidabilité).
e. **Cadre juridique par option** : applique `ncg-note-juridique` (3 couches,
...
[Synthèse issue de ncg-note-juridique : majorité par option, délais, incertitudes
...
`ncg-redaction-livrable` : nettoyage selon le destinataire, traçabilité sobre,
logo NCG, proposition d'export en une seule question fermée. La structure de la
...
| Besoin | Skill / tool |
|---|---|
| Majorité applicable, validité, délais, lecture du RCP | `ncg-note-juridique` (substance, réserves incluses) |
| Mise au propre, version externe, export Word | `ncg-redaction-livrable` (forme, nettoyage, logo) |
