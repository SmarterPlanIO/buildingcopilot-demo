...
description: >-
  ...
  question juridique (→ ncg-note-juridique). L'analyse constate et localise ;
  elle ne recommande pas d'action engageante sans instruction dédiée
  (→ ncg-fiche-decision).
---

# Analyse de portefeuille — Assistant Copro NCG
...
`ncg-redaction-livrable` ; l'instruction d'une décision, de `ncg-fiche-decision`.
...
- Si l'analyse débouche sur une décision à prendre (renégociation, travaux,
  ...
  `ncg-fiche-decision` — ne pas trancher ici.
